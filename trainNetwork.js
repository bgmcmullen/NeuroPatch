...
function testNetwork(input, weights, inputs, outputs, activationFunction, outputFunction) {
  for (var i = 0; i < weights.length; i++) {
    inputs[i] = dotProductMatricies(i < 1 ? input : outputs[i - 1], weights[i]);
...
function backpropagation(error, weights, gradients, inputs, outputs, derivativeFunction, outputDerivative) {
  for (var i = weights.length - 1; i >= 0; i--) {
    gradients[i] = multiplyMatricies(i === weights.length - 1 ? error :
      dotProductMatricies(gradients[i + 1], transposeMatrix(weights[i + 1])),
...
function updateWeights(input, outputs, weights, gradients, learningRate) {
  for (var i = weights.length - 1; i >= 0; i--) {
    weights[i] = addMatricies(weights[i], multiplyMatrixByFloat(dotProductMatricies(transposeMatrix(i < 1 ? input : outputs[i - 1]), gradients[i]), learningRate));
  }
}

function train(inputMatrix, outputMatrix, hiddenSizes, epochs, learningRate, activationFunction, derivativeFunction, weights, outputFunction, outputDerivative) {
  var X = inputMatrix;
  var y = outputMatrix;
...
function trainNetwork(input, output, layers, epochs, learningRate, activation, weights, outputActivation){
	...
	var weights = train(X, y, layers, epochs, learningRate, activationFunction, derivativeFunction, weights, outputFunction, outputDerivative);
